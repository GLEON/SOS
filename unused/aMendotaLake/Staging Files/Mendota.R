setwd('~/Documents/Rpackages/SOS/aMendotaLake/Staging Files/')
...
write.csv(o2_out,'../aMendotaValidationDO.csv',row.names = F,quote=F)
######################## TP and DOC ########################
...
chem2 = chem %>% select(date = sampledate, doc = doc, tp = totpuf_sloh) %>%
  group_by(date) %>%
  dplyr::summarise(docMean = mean(doc,na.rm=T),tp = mean(tp,na.rm=T)) %>%
  ungroup
...
chem3 = chem %>% select(date = sampledate,depth, doc = doc) %>%
  ...
  group_by(date) %>%
  dplyr::summarise(docSurf = mean(doc,na.rm=T)) %>%
  ungroup
...
write.csv(chem5,'../aMendotaValidationDOC.csv',row.names = F,quote=F)
######################## SECCHI ########################
...
######################## INFLOW ########################
inflow = read.csv('USGS-05427850_NWIS_Yahara_Highway113.csv',stringsAsFactors = F)
...
inflow2$discharge = inflow2$discharge * 1.5
inflow2$DOC = inflow2$discharge * 2 + 6
inflow2$DOC[inflow2$DOC > 15] = 15
...
write.csv(rain,'../aMendotaRain.csv',row.names = F,quote=F)

######################## COMBINE DATAFRAMES ########################
df = data.frame(datetime = seq.Date(as.Date('2002-03-28'),as.Date('2014-12-31'),by='day'))
df$Volume = vol
...
df$SW_DOC[df$date %in% inflow2$date] = inflow2$DOC[inflow2$date %in% df$date]
df$Secchi[df$datetime %in% secchi2$date] = secchi2$secchi[secchi2$date %in% df$datetime]

write.csv(df,'../aMendotaTS.csv',row.names = F,quote = F)
