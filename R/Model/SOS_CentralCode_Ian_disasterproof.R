...
TimeSeriesFile <- "H:/Ian_GIS/gleon/SOS/TroutLake/TestDataTrout.csv"
ParameterFile <- "H:/Ian_GIS/gleon/SOS/TroutLake/ParameterInputsTrout.txt"
############################################
...
##### LOAD FUNCTIONS #######################
setwd('H:/Ian_GIS/gleon/SOS/R/Model')
source("SOS_Sedimentation.R")
source("SOS_SWGW.R")
source("SOS_GPP.R")
source("SOS_Resp.R")
############################################
...
# POC_flux
POC_flux$Date = OutputTimeSeries
POC_flux$Date = as.Date(POC_flux$Date, "%m/%d/%Y") # ignore stupid time zone warning
locs = tapply(X=POC_flux$Date, FUN=min, INDEX=format(POC_flux$Date, '%Y%m'))
at = POC_flux$Date %in% locs

# POC_conc
POC_conc$Date = ConcOutputTimeSeries
POC_conc$Date = as.Date(POC_conc$Date, "%m/%d/%Y") # ignore stupid time zone warning
locs = tapply(X=POC_conc$Date, FUN=min, INDEX=format(POC_conc$Date, '%Y%m'))
at = POC_conc$Date %in% locs

# DOC_flux
DOC_flux$Date = OutputTimeSeries
DOC_flux$Date = as.Date(DOC_flux$Date, "%m/%d/%Y") # ignore stupid time zone warning
#DOC_flux$Month = month(DOC_flux$Date, label=T, abbr=T)
locs = tapply(X=DOC_flux$Date, FUN=min, INDEX=format(DOC_flux$Date, '%Y%m'))
at = DOC_flux$Date %in% locs

# DOC_conc
DOC_conc$Date = ConcOutputTimeSeries
DOC_conc$Date = as.Date(DOC_conc$Date, "%m/%d/%Y") # ignore stupid time zone warning
locs = tapply(X=DOC_conc$Date, FUN=min, INDEX=format(DOC_conc$Date, '%Y%m'))
at = DOC_conc$Date %in% locs

#Plot POC and DOC fluxes in standardized units (g/m2/yr)
xlabel <- "Month/Year"
#xlabel_Ian = c(OutputTimeSeries[1],OutputTimeSeries[101],OutputTimeSeries[201],OutputTimeSeries[301],OutputTimeSeries[401],OutputTimeSeries[501])
...
for (n in 1:4){
  plot(POC_flux[,n]~POC_flux[,5],xaxt='n',xlab=xlabel,ylab=ylabelPOC[n],type='l')
       axis(side=1, at=POC_flux$Date[at], labels=format(POC_flux$Date[at], '%b-%y'))
}

for (n in 1:5){
  plot(DOC_flux[,n]~DOC_flux[,6],xaxt='n',xlab=xlabel,ylab=ylabelDOC[n],type='l')
    axis(side=1, at=DOC_flux$Date[at], labels=format(DOC_flux$Date[at], '%b-%y'))  
}

#POC and DOC concentration in time (g/m3)
plot(POC_conc[,1]~POC_conc[,2],xaxt='n',xlab=xlabel,ylab="POC Conc (g/m3)",type="l")
axis(side=1, at=POC_conc$Date[at], labels=format(POC_conc$Date[at], '%b-%y'))

plot(DOC_conc[,1]~DOC_conc[,2],xaxt='n',xlab=xlabel,ylab="DOC Conc (g/m3)",type="l")
axis(side=1, at=DOC_conc$Date[at], labels=format(DOC_conc$Date[at], '%b-%y'))
