...
flux_plot = function(LakeName, ylim1, ylim2, legend){
  ...
  #poc
  POC_FlowIn_gm2y=POC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(FlowIn_gm2y))
  POC_FlowIn_gm2y$day = rep(1,1,nrow(POC_FlowIn_gm2y))
  POC_FlowIn_gm2y$month = as.numeric(POC_FlowIn_gm2y$month)
  POC_FlowIn_gm2y$year = as.numeric(POC_FlowIn_gm2y$year)
  colnames(POC_FlowIn_gm2y) = c('month','year','FlowIn_gm2y','day')
  month_day_year = paste0(POC_FlowIn_gm2y$month,'/',POC_FlowIn_gm2y$day,'/',POC_FlowIn_gm2y$year)
  POC_FlowIn_gm2y = POC_FlowIn_gm2y[3]
  
  POC_NPPin_gm2y=POC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(NPPin_gm2y))
  colnames(POC_NPPin_gm2y) = c('month','year','NPPin_gm2y')
  POC_NPPin_gm2y = POC_NPPin_gm2y[3]
  
  POC_FlowOut_gm2y=POC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(FlowOut_gm2y))
  colnames(POC_FlowOut_gm2y) = c('month','year','FlowOut_gm2y')
  POC_FlowOut_gm2y = POC_FlowOut_gm2y[3]
  
  POC_sedOut_gm2y=POC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(sedOut_gm2y))
  colnames(POC_sedOut_gm2y) = c('month','year','sedOut_gm2y')
  POC_sedOut_gm2y = POC_sedOut_gm2y[3]
  ...
  #DOC
  DOC_FlowIn_gm2y=DOC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(FlowIn_gm2y))
  DOC_FlowIn_gm2y$day = rep(1,1,nrow(DOC_FlowIn_gm2y))
  DOC_FlowIn_gm2y$month = as.numeric(DOC_FlowIn_gm2y$month)
  DOC_FlowIn_gm2y$year = as.numeric(DOC_FlowIn_gm2y$year)
  colnames(DOC_FlowIn_gm2y) = c('month','year','FlowIn_gm2y','day')
  month_day_year = paste0(DOC_FlowIn_gm2y$month,'/',DOC_FlowIn_gm2y$day,'/',DOC_FlowIn_gm2y$year)
  DOC_FlowIn_gm2y = DOC_FlowIn_gm2y[3]
  
  DOC_NPPin_gm2y=DOC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(NPPin_gm2y))
  colnames(DOC_NPPin_gm2y) = c('month','year','NPPin_gm2y')
  DOC_NPPin_gm2y = DOC_NPPin_gm2y[3]
  
  DOC_FlowOut_gm2y=DOC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(FlowOut_gm2y))
  colnames(DOC_FlowOut_gm2y) = c('month','year','FlowOut_gm2y')
  DOC_FlowOut_gm2y = DOC_FlowOut_gm2y[3]
  
  DOC_respOut_gm2y=DOC_df %>%
    mutate(month = format(Date, "%m"), year = format(Date, "%Y")) %>%
    group_by(month, year) %>%
    summarise(mean = mean(respOut_gm2y))
  colnames(DOC_respOut_gm2y) = c('month','year','respOut_gm2y')
  DOC_respOut_gm2y = DOC_respOut_gm2y[3]
