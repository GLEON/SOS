...
BurialFactor_init <- parameters[row.names(parameters)=="BurialFactor",1] #(1/days) Parameter estimation of OC burial in sediments
############################################
...
DOC_miner_const_init <- parameters[row.names(parameters)=="DOC_miner_const",1] #
RespParam_init <- DOC_miner_const <- parameters[row.names(parameters)=="RespParam",1] #
############################################
...
calcModelNLL <- function(BF,RP,DOC,steps,ValidationData){
  modeled = modelDOC(BF,RP,DOC,steps)
  
  obsIndx = ValidationData$datetime %in% modeled$datetime
  modIndx = modeled$datetime %in% ValidationData$datetime
  
  CalibrationOutput <- data.frame(datetime = ValidationData[obsIndx,]$datetime,
                           Measured = ValidationData[obsIndx,]$DOC, Modelled = modeled[modIndx,]$DOC_conc)
  res = CalibrationOutput$Measured - CalibrationOutput$Modelled
...
toOptim <- function(params){
  calcModelNLL(params[1], params[2], params[3], steps, ValidationData)
}

## Optimization of GPP and R using optim
optimOut = optim(par = c(BurialFactor_init,RespParam_init,DOC_miner_const_init), fn = toOptim) #control = list(maxit = 100)

## New parameters from optimization output
BurialFactor <- optimOut$par[1] #
RespParam <- optimOut$par[2]
DOC_miner_const <- optimOut$par[3] #
