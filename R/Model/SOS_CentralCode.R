...
##### Sub-Topic Parameters: GPP/NPP ############
#Currently built into GPP/NPP sub code.
############################################
...
NPPdata <- data.frame(NPP=numeric(steps),NPP_mass=numeric(steps))
############################################
...
MinRespData <- data.frame(DOC_miner_mass=numeric(steps))
############################################
...
for (i in 1:(steps)){
  ...
  #Call NPP Function
  NPPdata$NPP[i] <- NPP(InputData$Chla[i],InputData$TP[i],InputData$SurfaceTemp[i]) #mg C/m^2/d
  NPPdata$NPP_mass[i] <- NPPdata$NPP[i]*lakeArea*TimeStep/1000 #g

  #Call SWGW Function
  SWGW <- SWGWFunction(Q_sw,Q_gw,Rainfall,Aoc_year, PC, lakePerim, Woc_year, PW, DOC_GW, prop_GW, 
  ...
  POC_flux$NPP_in[i] <- NPPdata$NPP_mass[i]/lakeArea/(TimeStep/365)
  ...
  POC_conc[i+1,1] <-  POC_conc[i,1] + ((NPPdata$NPP_mass[i] + SWGW_mass_in$POC[i] - POC_outflow[i,1] - POC_sed_out[i,1])/lakeVol) #g/m3
  DOC_conc[i+1,1] <-  DOC_conc[i,1] + ((SWGW_mass_in$DOC[i] - DOC_outflow[i,1] - MinRespData$DOC_miner_mass[i])/lakeVol) #g/m3
  #! Missing mineralization of DOC by microbes (HD and PH)
  
  #POC and DOC load (in) and fate (out) (g)
  POC_load$total[i] <- NPPdata$NPP_mass[i] + SWGW_mass_in$POC[i] #g 
  POC_load$alloch[i] <- SWGW_mass_in$POC[i] #g
  POC_load$autoch[i] <- NPPdata$NPP_mass[i] #g
