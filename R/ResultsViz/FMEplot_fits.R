...
png(paste0('R/ResultsViz/Figures/FMEfit_all2.png'),width = 8,height = 11,units = 'in',res = 300)
  ...
  for (l in 1:5) {
    ...
    plot(modelDOC$datetime,modelDOC$DOC,type='o',xlab='',ylab = expression(paste("DOC (mg L"^"-1",")")),pch=21,
         bg='cyan4',main=lakenames[l])
    # lines(modelDOC$datetime,modelDOC$DOCwc,type='o',col='grey50',pch=16)
    lines(modelDOC$datetime,modelDOC$Modelled,type='o',bg='firebrick',pch=22)

    abline(v = seq.POSIXt(as.POSIXct('1990-01-01'),as.POSIXct('2015-01-01'),by='year'),col='grey80',lty=2)
    if (l == 1){
      legend('topleft',legend = c('Observed','Modeled'),pt.bg=c('cyan4','firebrick'),pch=c(21,22),cex=0.8,lwd = 1,bg = 'white')
    }
    
    plot(modelDO$datetime,modelDO$DO_con,xlab='',type='o',ylab = expression(paste("DO (mg L"^"-1",")")),pch=21,main=lakenames[l],
         ylim = range(modelDO[,c(3,7)]),bg='cyan4')
    lines(modelDO$datetime,modelDO$Conc_Modelled,type='o',bg='firebrick',pch=22)
    abline(v = seq.POSIXt(as.POSIXct('1990-01-01'),as.POSIXct('2015-01-01'),by='year'),col='grey80',lty=2)
