...
InflowData <- data.frame(Q_in=numeric(steps), Time=numeric(steps), DOC_SW=numeric(steps), DOC_GW=numeric(steps), DOC_SWGW=numeric(steps),POC_SWGW=numeric(steps))

#Inputs- Lake Specific
prop_GW <- .3 # Unitless: proportion of Q_in that is from groundwater
DOC_GW <- 5 # g/m3: DOC concentration in groundwater
...
for (i in 1:(EndDay-StartDay)){
  ...
  InflowData$Time[i] <-  86400 * TimeStepConversion # convert time from seconds to days
  
  InflowData$DOC_SW[i] <- InflowData$Q_in[i] * (1-prop_GW) * DOC_SW
  InflowData$DOC_GW[i] <- InflowData$Q_in[i] * prop_GW * DOC_GW
 
  InflowData$DOC_SWGW[i] <- (InflowData$DOC_SW[i] + InflowData$DOC_GW[i]) * InflowData$Time[i] # DOC inflow
  # g/d
