...
#! These are Hilary's and Paul's notes
#! Consider changing parameter file to matrix with nrows = nlakes and ncols = nparams; put units in header
############################################
...
#! Consider tracking masses rather than concentrations, and output concentrations only for visualizing
...
for (i in 1:(steps)){
  ...
  #Call SWGW Function
  #! The "output" in the variable name is misleading.  This is actually an input of OC to the lake, yes?
  ...
  #! See the SWGW... function for edits in what is being tracked for POC
  ...
  #! For the first day, the total loads are about 0.055 g/m2LA/d or ~15 g/m2LA/y, which is good! (LA = lakearea)
  ...
  #! Not clear, but seems like processes other than burial are in the "SedimentationFunction" function
  #! Inconsistent on how functions deal with time step adjustments -- sometimes internally, sometimes in the central code
  ...
  #! Saving everything on a per year unit seems weird
  ...
  #! Hilary and Paul's DOC mineralization klug;  needs to be added to overall housekeeping
  DOC_Mineralization_mass = DOC_conc[i,1] * lakeVol * 0.005 # Current concentration multiplied by lakevolume and a mineralization constant in units of 1/d
  ...
  #! Missing mineralization of DOC by microbes
  DOC_conc[i+1,1] <-  DOC_conc[i,1] + ((SWGW_mass_in$DOC[i] - DOC_outflow[i,1] - DOC_Mineralization_mass)/lakeVol) #g/m3
  ...
  #! Changed the out of DOC to incluce mineralization
  #! DOC_out$total[i] <- DOC_outflow[i,1] #g
  DOC_out$total[i] <- DOC_outflow[i,1] + DOC_Mineralization_mass #g
